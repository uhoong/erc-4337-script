...
const verifyingPaymaster = (paymasterAddr,offchainSigner) => (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    const signer = new ethers_1.ethers.Wallet(offchainSigner);

    ctx.op.verificationGasLimit = ethers_1.ethers.BigNumber.from(ctx.op.verificationGasLimit).mul(3);

    // let address = ethers_1.ethers.utils.arrayify(paymasterAddr)
    let data = yield signer.signMessage(ethers_1.ethers.utils.arrayify(ctx.getPartialUserOpHash())).then((data)=>{
        return paymasterAddr+data.slice(2,data.length);
    })
    ctx.op.paymasterAndData = data;
});
